...
def ps_set_appointment():
    ...
    if get_name and get_surname and get_age and get_symptoms:
        if (get_age.isdigit() == True) and ((get_name.isalpha() and get_surname.isalpha()) == True):
...
def main_proj():
    ...
    mainscreen.geometry(f"400x200+{int(xBor + 500)}+{int(yBor + 300)}")
    # ybor + 200 -> ybor + 300, size from 400x400 -> 400x200
    mainscreen.title("DOCTOR APPOINTMENT SIMULATOR")

    Label(text="Doctor Appointment Simulator", fg="white", bg="#526D82", bd=15, font=("Sans Serif", 15, "bold"),
          padx=30).pack(pady=20)

    Button(text="START", fg="white", bg="#526D82", height=2, width=20, bd=5,
           font=("Sans Serif", 15, "bold"), command=ps_ds_qs).place(x=75, y=100)
